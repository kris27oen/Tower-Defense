		...
		if (preview->id == 3 || preview->id == 4) {
            ...
            if (remainId != -1) {
                if (remainId == 0)
                    ...
                    preview = new Giant(0, 0);
                else if (remainId == 3)
                    preview = new Freeze(0, 0);
                else if (remainId == 4)
                    preview = new Rage(0, 0);
...
void PlayScene::ReadMap() {
	...
	while (fin >> c) {
        ...
        case '3': mapData.push_back(TILE_FASTCANNON); break;
		...
		for (int j = 0; j < MapWidth; j++) {
            ...
            switch (md) {
                ...
                case TILE_CANNON:
                    ...
                    break;
                case TILE_FASTCANNON:
                    DefenseGroup->AddNewObject(new CannonFast(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                    break;
                case TILE_TRAP:
                    TrapGroup->AddNewObject(new Trap(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                    break;
                case TILE_FLOOR:
                    if (j <= MapWidth-2 && j >= 2) {
...
bool PlayScene::CheckOccupied(int x, int y) {
    ...
    if (tt == TILE_WALL || tt == TILE_CANNON || tt == TILE_FASTCANNON) return true;
